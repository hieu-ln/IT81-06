...
int chuyenDoiThapPhanSangNhiPhan(int so)
{
	khoiTao(a, sp);
	int np = 0;
	while(so != 0)
	{
		int du = so % 2;
		if(them1PhanTu(a, sp, du))
			so /= 2;
		else
			break;
	}
	while(!kiemTraRong(a, sp))
	{
		int x;
		if(xoa1PhanTu(a, sp, x))
			np = np * 10 + x;
		else
			break;
	}
...
int main()
{
	...
	do{
		...
		case 7:
			...
			cin >> tp;
			cout << "Gia tri nhi phan tuong ung la: " << chuyenDoiThapPhanSangNhiPhan(tp) << endl;
			break;
		}
